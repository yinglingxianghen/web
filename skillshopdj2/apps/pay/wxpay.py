...
class WxPayConf_pub(object):
    ...
    BUY_COURSES_SUCCESS_HREF_URL="xxxxxxxx"
    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    #NOTIFY_URL = "xxxxxxxxshoppingcart/nativenotify"
    NOTIFY_URL = "xxxxxxxx/nativenotify/"
    #=======【JSAPI路径设置】===================================
    #获取access_token过程中的跳转uri，通过跳转将code传入jsapi支付页面
    JS_API_CALL_URL = "xxxxxxxx/wxpay/?showwxpaytitle=1"

    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    #SSLCERT_PATH = "/common/djangoapps/weixin_pay/cacert/apiclient_cert.pem"
    #SSLKEY_PATH = "/common/djangoapps/weixin_pay/cacert/apiclient_key.pem"
    #上面为php版本专用
    #下面为其他语言使用
    #SSLCERT_PATH = "/edx/app/edxapp/edx-platform/lms/djangoapps/weixin_pay/cacert/apiclient_cert.p12"
    PAY_DIR = os.path.dirname(os.path.abspath(__file__))
    SSLCERT_PATH = os.path.join(PAY_DIR,'cacert/apiclient_cert.p12')
    ...
    def postXmlSSL(self, xml, url, second=30, cert=True, post=True):
        ...
        try:
            ...
            if cert:
                ...
                self.curl.setopt(pycurl.SSLCERTTYPE, "p12")
...
class Common_util_pub(object):
    ...
    def getSign(self, obj):
        ...
        #签名步骤二：在string后加入KEY
        String = "{0}&key={1}".format(String,WxPayConf_pub.KEY).encode("utf-8")
        #签名步骤三：MD5加密
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        """生成接口参数xml"""
        #检测必填参数
        if any(self.parameters[key] is None for key in ("out_trade_no", "body","detail", "total_fee",  "trade_type")):
        ...
        self.parameters["appid"] = WxPayConf_pub.APPID  #公众账号ID
            #self.parameters["appId"] = WxPayConf_pub.APPID  # 公众账号ID
        self.parameters["mch_id"] = WxPayConf_pub.MCHID  #商户号
        self.parameters["spbill_create_ip"] = "119.29.33.143"  #终端ip
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["notify_url"] = WxPayConf_pub.NOTIFY_URL  # 返回消息url
        self.parameters["sign"] = self.getSign(self.parameters)  # 签名

        #self.parameters["openid"] = self.getOpenid()
...
class NativeCall_pub(Wxpay_server_pub):
    ...
    def createXml(self):
        ...
        if self.returnParameters["return_code"] == self.SUCCESS:
            ...
            self.returnParameters["nonce_str"] = self.createNoncestr() #随机字符串
            self.returnParameters["sign"] = self.getSign(self.returnParameters) #签名
            self.returnParameters["return_msg"] =''
            self.returnParameters["err_code_des"] = ''
...
#class WeiXinAdapter(Common_util_pub):
    #def sendPicAndMessageToUser(self,token,arr):
        #xml=self.arrayToXml(arr)
        #requestUrl ="https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=%s" %(token)
        #return self.postXmlCurl(xml,requestUrl)

def test():
    c = HttpClient()
    assert c.get("http://www.careeru.cn")[:15] == "<!DOCTYPE html>"
    c2 = HttpClient()
    assert id(c) == id(c2)
