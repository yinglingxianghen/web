...
from bbs_demo.custompaginator import CustomPaginator
...
def index(request):
    ...
    paginator =  CustomPaginator(current_page, 11, posts, 10)
    user = request.user
    if user.is_authenticated():
        bbs_user = BBS_User.objects.filter(user=user)[0]
    else:
        bbs_user = None
    ...
    return render(request, 'index.html', {'category_list': category_list, 'post_list':post_list, 'current_page':int(current_page), 'bbs_user':bbs_user})
...
def category(request,category_id):
    ...
    print(user)
    if user.is_authenticated():
        bbs_user = BBS_User.objects.filter(user=user)
    else:
        bbs_user = None
    category_list = Category.objects.all()
