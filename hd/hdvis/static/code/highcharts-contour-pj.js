...
seriesTypes.contour = extendClass(seriesTypes.heatmap, {
    ...
    drawTriangle: function (triangle_data, edgeCount, show_edges, contours, show_tickValues) {
        var fill;
        ...
        for (var contour_index=0; contour_index<contours.length; contour_index++) {
            ...
            if (show_tickValues){
                for (var tickIndex in show_tickValues) {
                    var tickValue = show_tickValues[tickIndex];
                    var contourVertexes=[];
                    for (var i=0; i<3; i++) {
    ...
    drawGraph: function () {
        ...
        for (i=0; i<triangle_count; i++) {
            series.drawTriangle(series.triangles[i], egde_count, options.showEdges, contours, options.showTickValues);
        }
    }
});
